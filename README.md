...
后端：Spring Boot 3.5 · Spring Security 6 · MyBatis-Plus 3.5 · MySQL 8.4 · Redis 7 · JWT (jjwt 0.12) · Docker Compose · JDK 17+（本机 JDK 25 亦可，字节码按 17 编译）
...
Spring Boot 后端（8080）
   ...
   │        登出即失效、服务端可踢人；禁用账号 → 删全部会话 + 角色缓存（禁用即踢下线，第 5 周）
...
| 本地全量回归 | `bash test-payloads/e2e-test.sh`（20 用例）+ `bash test-payloads/redis-e2e-test.sh`（11 用例）+ `bash test-payloads/user-e2e-test.sh`（25 用例） | RBAC/CRUD + Redis 会话/角色变更/登出 + 用户管理/禁用踢下线 |
...
| PUT | /api/users/{id}/status | 启用/禁用账号（仅 ADMIN；禁用 = 删全部会话踢下线 + 删角色缓存） |
| PUT | /api/users/{id}/roles | 覆盖式分配角色（仅 ADMIN；改完旧 token 即时生效） |
| GET | /api/roles | 全部启用角色（仅 ADMIN，供角色下拉框） |

内置账号：`admin / 123456`（ADMIN）、`zhangsan / 123456`（EMPLOYEE，需注册）。角色-权限、部门/员工表结构与种子见 `sql/init.sql`。
...
├── controller/   # Auth / Dept / Employee / User / Role
...
└── dto/          # RegisterDTO/LoginDTO/UserVO/EmployeeVO 等
...
test-payloads/       # 端到端回归脚本（e2e 20 + redis 11 + user 25；token 文件不入 git）
```
